...
public interface DialogueDetailMapper {
	@Insert("insert into dialogue_detail(USER_ID,CREATE_TIME,STATUS,CHANNEL_ID) values" +
			"(#{userId},#{createTime},#{status},#{channelId})")
	int add(DialogueDetail dialogueDetail);
	
	@Update("update dialogue_detail set USER_ID=#{userId},STATUS=#{status},CREATE_TIME=#{createTime}," +
